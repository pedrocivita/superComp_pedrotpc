...
void BronKerboschPivotParallel(std::vector<std::vector<int>>& grafo,
                               std::set<int> R,
                               std::set<int> P,
                               std::set<int> X,
    ...
    for (size_t i = 0; i < P_sem_Nu.size(); ++i) {
        ...
        BronKerboschPivotParallel(grafo, R_v, P_intersec, X_intersec, cliqueMaximaLocal, depth + 1);
...
int main() {
    ...
    BronKerboschPivotParallel(grafo, R, P, X, cliqueMaxima);
